...
def australia_without_SA():
    ...
    var_domains = [["red", "green"] for region in regions]
    must_have_distinct_colors = lambda _, color: color[0] != color[1]
    constraints = [
        ((0, 1), must_have_distinct_colors),
        ((1, 2), must_have_distinct_colors),
...
class TestCSP(object):
    ...
    def test_assign_variable(self, australia):
        australia.assign_variable(5, "red", prune_var_domains=False)
    ...
    def test_prune_var_domains(self, australia):
    ...
    def test_prune_var_domains_in_tree_csp(self, australia_without_SA):
        csp = australia_without_SA
        csp.assign_variable(5, "red")
        assert csp.pruned_var_domains[0] == ["red", "green"]
        assert csp.pruned_var_domains[1] == ["red", "green"]
        assert csp.pruned_var_domains[2] == ["red", "green"]
        assert csp.pruned_var_domains[3] == ["red", "green"]
        assert csp.pruned_var_domains[4] == ["red", "green"]
        assert csp.pruned_var_domains[5] == ["red"]
        assert csp.assignment == {5: "red"}
        csp.assign_variable(0, "red")
        assert csp.pruned_var_domains[0] == ["red"]
